/**
 * Raw-token-aware Python `str()` rendering for the oracle (design D14).
 *
 * The runner's D13 binding recovers the float-vs-int distinction for the
 ...
 * `pythonCompat` semantics (`pythonStr` / `pythonRepr` /
 * `pythonFloatStr`) so no Python semantics are re-derived here — only
 * the structural container assembly (`[a, b]` / `{'k': v}`), which is
