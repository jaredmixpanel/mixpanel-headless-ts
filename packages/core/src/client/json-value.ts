 ...
 *
 * @remarks
 * Preserves the integer-vs-float distinction (`18` vs `18.0`) and the
 * exact digits of integers too large for a JS double, both of which the
 * canonicalizer and the `PRECISION_LOSS` verdict require. Construct it
 * with the verbatim token; an invalid token throws.
 * @example
 * ```typescript
 * const n = new JsonNumber("9007199254740993");
 * n.isIntegerToken(); // true
 * n.isUnsafeInteger(); // true — 2^53 + 1 does not round-trip a double
 * new JsonNumber("18.0").isIntegerToken(); // false
 * ```
 */
export class JsonNumber {
   ...
   * @throws {@link Error} - When `raw` is not a valid JSON number token.
   */
  ...
  /**
   * Report whether the token is an integer token (no fraction or exponent
   * part).
   *
  ...
  /**
   * Return the token's value as a JS double (correctly rounded, possibly
   * lossy).
   *
  ...
  /**
   * Report whether this is an integer token whose value cannot be
   * represented exactly as a JS double (magnitude above 2^53).
   *
 ...
 * @example
 * ```typescript
 * const row = { 10: "b", a: "x" }; // JS enumerates "10" first
 * attachKeyOrder(row, ["a", "10"]);
 * orderedKeys(row); // ["a", "10"]
 * ```
 */
...
/**
 * Return an object's keys in source order: the {@link LOSSLESS_KEY_ORDER}
 * sidecar when present, else `Object.keys` (which already equals source
 * order for objects without out-of-order integer-like keys).
 *
 * @param value - Any plain object (parsed or hand-built).
 * @returns The keys in Python-dict order.
 * @example
 * ```typescript
 * orderedKeys(parseLossless('{"b": 1, "2": 2}')); // ["b", "2"]
 * Object.keys(parseLossless('{"b": 1, "2": 2}')); // ["2", "b"]
 * ```
 */
...
/**
 * Return `Object.entries` in source order (see {@link orderedKeys}).
 *
 * @param value - Any plain object (parsed or hand-built).
 * @returns `[key, value]` pairs in Python-dict order.
 * @example
 * ```typescript
 * orderedEntries(parseLossless('{"b": 1, "2": 2}'));
 * // [["b", JsonNumber("1")], ["2", JsonNumber("2")]]
 * ```
 */
...
export interface ToNativeJsonOptions {
   ...
   * `"round"` — the correctly-rounded double, `"bigint"` — the exact
   * `bigint`.
   *
   * @defaultValue `"round"`
   */
 ...
 * narrowings: by default integer tokens beyond 2^53−1 double-round
 * (Python keeps the exact int), and float-ness of integral tokens is
 * erased (`42.0` → `42`; Pydantic-lax accepts both identically at every
 * consuming field). The first narrowing is opt-out: `"bigint"` maps an
 ...
 * lookup-table members use it because Mixpanel's
 * `data_group_id`s are negative int64s (`-8644926364725811123`) that a
 * double would round.
 ...
 * @example
 * ```typescript
 * toNativeJson(parseLossless('{"n": 18.0, "big": 9007199254740993}'));
 * // { n: 18, big: 9007199254740992 }
 * toNativeJson(parseLossless('[9007199254740993]'), { unsafeIntegers: "bigint" });
 * // [9007199254740993n]
 * ```
 */
...
/**
 * Convert a lossless object row to a native record — the record-typed
 * twin of {@link toNativeJson} that the service parsers use so each site
 * does not repeat the same assertion.
 *
 * @param value - A lossless object row.
 * @returns The native record.
 * @example
 * ```typescript
 * toNativeRecord(parseLossless('{"id": 7}')); // { id: 7 }
 * ```
 */
