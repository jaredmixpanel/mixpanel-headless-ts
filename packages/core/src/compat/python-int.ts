/**
 * Python `int(str)` parse grammar, implemented once here; consumed by
 * `Retry-After` parsing and lax coercion.
 *
 ...
 * @returns The coded error (message text is not part of the contract).
 */
...
/**
 * Parse a string exactly as CPython `int(str)` does.
 *
 ...
 *
 * Divergence (listed in PORTING.md): CPython returns arbitrary-precision
 * ints; this port returns a JS `number` and rejects results beyond
 * ±(2^53 − 1) with a coded error — the canonicalizer's 2^53 policy. No
 * caller can produce such a value legitimately; `Retry-After` is clamped
 * at 60 downstream.
 *
 * @param text - The string to parse.
 * @returns The parsed integer as a JS `number` (`-0` normalizes to `0`,
 *   matching Python `int`, which has no negative zero).
 * @throws {@link MixpanelHeadlessError} - Code `PY_INT_INVALID_LITERAL` when the
 *   input is not a valid CPython base-10 integer literal; code
 *   `PY_INT_UNSAFE_INTEGER` when the parsed magnitude exceeds 2^53 − 1.
 * @throws {@link TypeError} - When `text` is not a string.
 * @example
 * ```typescript
 * pythonInt("  1_5  "); // 15
...
export function pythonInt(text: string): number {
  ...
  if (magnitude > MAX_SAFE) {
    // Divergence: CPython ints are unbounded; the port rejects beyond 2^53 - 1.
...
/**
 * Coerce an arbitrary value exactly as CPython `int(value)` does — the
 * coercion ladder, as opposed to {@link pythonInt}, which is the
 * `int(str)` grammar alone. The replay walker's and analyzer's
 ...
 * @returns The integer.
 * @throws {@link MixpanelHeadlessError} - Code `PY_INT_INVALID_LITERAL` /
 ...
 *   failures and the code is the contract).
 * @throws {@link TypeError} - For a non-coercible type.
 * @example
 * ```typescript
