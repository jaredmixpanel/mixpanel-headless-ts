/**
 * Python `float(str)` parse grammar, implemented once here; consumed by
 * `Retry-After` handling and lax coercion.
 *
 ...
 * the CPython probes: `DIGITS ['.' [DIGITS]]` or `'.' DIGITS`
...
/**
 * Parse a string exactly as CPython `float(str)` does.
 *
 ...
 *   (`"1e400"` gives `Infinity`, exactly like CPython). Negative zero is
 *   preserved (`"-0.0"` gives `-0`).
