// `Workspace` bookmark CRUD: list/get/create/update/delete, the bulk
// operations, linked-dashboard ids and history, plus client-side params
// validation and warning logging. Mirrors `TestWorkspaceBookmarkCRUD` of
// `tests/unit/test_workspace_crud.py`; `httpx.MockTransport` becomes the
// injected-fetch seam and `caplog` the injected `WorkspaceLogger` sink.
...
// =============================================================================
// Workspace bookmark CRUD
// =============================================================================

describe("Workspace bookmark CRUD", () => {
  // python: TestWorkspaceBookmarkCRUD
  it("listBookmarksV2() returns list of Bookmark objects", async () => {
  ...
  it("listBookmarksV2() returns empty list when none exist", async () => {
    const { ws } = makeFacadeWorkspace(() => ok([]));
    await expect(ws.listBookmarksV2()).resolves.toStrictEqual([]);
  });

  it("listBookmarksV2(bookmark_type='funnels') passes filter", async () => {
  ...
  it("listBookmarksV2() preserves the API response order", async () => {
    const { ws } = makeFacadeWorkspace(() =>
  ...
  it("createBookmark() sends description when provided", async () => {
    const { ws } = makeFacadeWorkspace((request) => {
  ...
  it("createBookmark() can associate with a dashboard", async () => {
    const { ws } = makeFacadeWorkspace((request) => {
      if (request.method === "PATCH") {
        return ok(dashboardJson(99));
      }
  ...
  it("createBookmark() PATCHes the report into the dashboard layout", async () => {
  ...
  it("createBookmark() raises when dashboard_id is missing", async () => {
  ...
  it("createBookmark() rejects malformed sorting before any API call", async () => {
    const calls: CapturedFetchRequest[] = [];
    const { ws } = makeFacadeWorkspace((request) => {
      calls.push(request);
      return ok({});
  ...
  it("updateBookmark() rejects malformed sorting before the API call", async () => {
    const calls: CapturedFetchRequest[] = [];
    const { ws } = makeFacadeWorkspace((request) => {
      calls.push(request);
      return ok({});
  ...
  it("updateBookmark() rejects malformed displayOptions", async () => {
    const calls: CapturedFetchRequest[] = [];
    const { ws } = makeFacadeWorkspace((request) => {
      calls.push(request);
      return ok({});
  ...
  it("updateBookmark() name-only partial does not false-reject", async () => {
    const calls: CapturedFetchRequest[] = [];
    const { ws } = makeFacadeWorkspace((request) => {
      calls.push(request);
  ...
  it("updateBookmark() with warning-only errors does NOT raise", async () => {
    const calls: CapturedFetchRequest[] = [];
    const { ws } = makeFacadeWorkspace((request) => {
      calls.push(request);
  ...
  it("createBookmark() logs warnings instead of dropping them", async () => {
  ...
  it("createBookmark() accepts the canonical valid sorting block", async () => {
    const { ws } = makeFacadeWorkspace((request) =>
      ok(
        request.method === "PATCH"
          ? dashboardJson(99)
  ...
  it("getBookmark() returns a single Bookmark by ID", async () => {
    const { ws } = makeFacadeWorkspace(() =>
  ...
  it("getBookmark() preserves extra fields", async () => {
    const { ws } = makeFacadeWorkspace(() =>
      ok({
  ...
  it("updateBookmark() returns the updated Bookmark", async () => {
    const { ws } = makeFacadeWorkspace(() =>
  ...
  it("updateBookmark() can update description", async () => {
    const { ws } = makeFacadeWorkspace(() =>
  ...
  it("updateBookmark() can update query params", async () => {
    const { ws } = makeFacadeWorkspace(() =>
      ok({
  ...
  it("deleteBookmark() returns None on success", async () => {
    const { ws } = makeFacadeWorkspace(() => ({ status: 204 }));
    await expect(ws.deleteBookmark(1)).resolves.toBeUndefined();
  });

  it("deleteBookmark() handles a 200 response", async () => {
    const { ws } = makeFacadeWorkspace(() => ok({}));
    await expect(ws.deleteBookmark(1)).resolves.toBeUndefined();
  });

  it("bulkDeleteBookmarks() returns None on success", async () => {
    const { ws } = makeFacadeWorkspace(() => ({ status: 204 }));
    await expect(ws.bulkDeleteBookmarks([1, 2])).resolves.toBeUndefined();
  });

  it("bulkDeleteBookmarks() works with a single ID", async () => {
    const { ws } = makeFacadeWorkspace(() => ({ status: 204 }));
    await expect(ws.bulkDeleteBookmarks([42])).resolves.toBeUndefined();
  });

  it("bulkDeleteBookmarks() sends multiple IDs", async () => {
  ...
  it("bulkUpdateBookmarks() returns None on success", async () => {
    const { ws } = makeFacadeWorkspace(() => ok({}));
    await expect(
  ...
  it("bulkUpdateBookmarks() handles multiple entries", async () => {
    const { ws } = makeFacadeWorkspace(() => ok({}));
    await expect(
      ws.bulkUpdateBookmarks([
  ...
  it("bulkUpdateBookmarks() can update query params", async () => {
    const { ws } = makeFacadeWorkspace(() => ok({}));
    await expect(
      ws.bulkUpdateBookmarks([
  ...
  it("bookmarkLinkedDashboardIds() returns list of int", async () => {
  ...
  it("bookmarkLinkedDashboardIds() returns [] when none linked", async () => {
    const { ws } = makeFacadeWorkspace(() => ok([]));
    await expect(ws.bookmarkLinkedDashboardIds(1)).resolves.toStrictEqual([]);
  });

  it("bookmarkLinkedDashboardIds() works with a single ID", async () => {
    const { ws } = makeFacadeWorkspace(() => ok([42]));
    await expect(ws.bookmarkLinkedDashboardIds(1)).resolves.toStrictEqual([42]);
  });

  it("getBookmarkHistory() returns BookmarkHistoryResponse", async () => {
    const { ws } = makeFacadeWorkspace(() =>
      ok({
        results: [
  ...
  it("getBookmarkHistory() handles empty history", async () => {
    const { ws } = makeFacadeWorkspace(() =>
  ...
  it("getBookmarkHistory() preserves pagination metadata", async () => {
    const { ws } = makeFacadeWorkspace(() =>
      ok({
  ...
  it("listBookmarksV2() maps 'type' to bookmark_type", async () => {
    const { ws } = makeFacadeWorkspace(() =>
  ...
  it("createBookmark() works with the funnel bookmark type", async () => {
    const { ws } = makeFacadeWorkspace((request) =>
      ok(
        request.method === "PATCH"
          ? dashboardJson(99)
