// Workspace.resolveReportLink: slug, bookmark and short links, scope and
// unsupported inputs, and slugs with an unknown server type. Mirrors the
// TestResolve* classes of tests/unit/test_workspace_report_links.py.
// `dataclasses.replace(resolved, …)` becomes a rebuild through the
// ResolvedReport constructor; error messages are not asserted, codes/details are.
