...
describe("iterJsonlLines", () => {
  ...
  it("authored-multibyte-char-split-mid-codepoint", async () => {
    ...
    expect(await lines([whole.slice(0, 12), whole.slice(12)])).toStrictEqual([
      '{"emoji": "😀"}',
    ]);
  });

  it("yields nothing for an empty stream", async () => {
    expect(await lines([])).toStrictEqual([]);
    expect(await lines([""])).toStrictEqual([]);
  });
  ...
  it("yields a BOM-only line verbatim (Python utf-8 codec keeps U+FEFF)", async () => {
    ...
    // ["\ufeff"], never []).
    expect(
      await lines([new Uint8Array([0xef, 0xbb, 0xbf, 0x0a])]),
    ).toStrictEqual(["\uFEFF"]);
  });
