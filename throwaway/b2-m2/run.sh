#!/usr/bin/env bash
# THROWAWAY (R10.9) — derandomised, re-runnable driver for the B2-M2
# (validation.py shard V1b) differential harness. Deleted by the B2
...
SEED="${1:-20260815}"
RUNS="${2:-600}"
node throwaway/b2-m2/harness.mjs --seed "$SEED" --runs "$RUNS" \
  >/dev/null 2>throwaway/b2-m2/harness-stderr.log
node -e '
const r = require("./throwaway/b2-m2/report.json");
