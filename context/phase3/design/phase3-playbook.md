...
- **R10.7 four-bug batch re-pin update (2026-08-16, follow-up per P3-7 trigger 3 /
  Phase-4 inbound-ledger row 2)**: corpus re-pinned `70c904d` → `700db99`
  (`ts-port/python-bugfix-batch`; the SINGLE re-pin event for Python fixes (a)
  frequency-filter clause shape, (b) dataGroupId threading, (c) 403-sniff TypeError,
  (d) OAuth error-details token redaction). Drift accounting CLEAN: 150 bundles
  stamp-only; exactly the 20 disclosed FIX-1 vectors modified (13 bug-(a) + 7
  bug-(b)); 11 vectors ADDED by the batch's new tests (bug (a) +1
  `bookmark_builders.build_frequency_filter_entry`, bug (c) +9
  `api_client.sign_replays`, bug (d) +1 `oauth_flow.refresh_tokens`); 0 removed,
  0 unexplained (manifest: recorded 3,031→3,042; `raw_transport_no_entrypoint`
  34→37 — the three non-recordable `TestTokenPayloadRedaction` cases). Re-measured
  totals: **3,262** vectors (was 3,251; prefix deltas exactly `bookmark_builders`
  134→135, `api_client` 810→819, `oauth_flow` 7→8). Python corpus runner:
  **3,262 PASS / 0 FAIL / 0 UNPORTED** at the new pin. Referee (b) now FULLY
  CLEAN (structural 314/314 ACCEPT; deep 125 ACCEPT / 0 REJECT / 189 SKIP — the
  2 standing frequency-filter deep REJECTs RETIRED). The P3-1 batch-gate rows are
  historical (Phase 3 closed); the standing expectation for Phase-4 burn-in and
  the TS follow-up sync is 3,251+Δ = **3,262**. TS follows per R10.7 flip
  discipline (bug-compat twins retire in the same change that flips tests/vectors;
  the pinned ajv dataGroupId REJECT set retires there).

- **R10.7 four-bug batch CLOSED (2026-08-17, batch gate)**: TS followed at
  `2b72ce1` (all four bug-compat twins retired in the same change that flipped
  tests/vectors; the 5 pinned ajv dataGroupId disclosures — 4 clause-level B3 +
  1 sections-level B5 — DELETED from `differential/test/bookmark-referee-feed.test.ts`,
  no allowlist kept), hardened by the doubled-blind arbiter commits
  (Python `20c7d6b`/`db8a33e`, TS `b7152da`/`da68958` — non-dict-200 guard +
  allowlist token redaction). **Disclosure registry state: NO standing disclosed
  REJECT remains on EITHER referee** — (a) ajv `npm run referee:bookmark` 9/9,
  0 REJECT; (b) bookmark_parser structural 314/314 ACCEPT, deep 125 ACCEPT /
  0 REJECT / 189 SKIP_NON_INSIGHTS. Any future referee REJECT is a NEW finding.
  Batch-gate verification: corpus **3,262 / 0 / 0 @ 700db996** in BOTH languages;
  differential regression 11 seeds (fresh 741097477 + the full B9 gate seed set)
  × 28,091 examples / 0 divergences. Gate record:
