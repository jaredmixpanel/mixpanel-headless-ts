...
All three share the same `Workspace` API — code written against core runs in either environment. `@mixpanel-headless/core` is a dependency of both platform packages and its public surface is re-exported from `@mixpanel-headless/node`, so a Node project needs one install and one import. Reach for `core` directly only when you already hold a resolved credential or are building your own platform layer.
