// The result under the workbench: a title line, the visualization the
...
const EMPTY: Readonly<Record<EngineKind, string>> = {
  ...
  aha: "Which behaviors do users come back for? Mixpanel has no report for this: it takes one retention query per candidate return event and a ranking, which is a loop.",
  matrix:
...
function resultTitle(spec: AnySpec, result: AnyResult | null): string {
  switch (spec.kind) {
    ...
    case "aha": {
      return `Behaviors users come back for after ${spec.born}`;
    }
    case "matrix": {
